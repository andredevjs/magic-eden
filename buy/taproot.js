...
const generateTaprootWallet = (keys) => {
  const ms = btc.p2tr_ms(
    1,
    keys.map((key) => key.publicKey.slice(1)),
  );

  const p2tr = btc.p2tr(undefined, { script: ms.script }, NETWORK, true);
  return { p2tr };
};
...
const generateRandomKeys = () => {
  ...
  return generateTaprootWallet(randomKeys);
};
...
const generateLocalKeys = () => {
  ...
  return generateTaprootWallet(localKeys);
};
